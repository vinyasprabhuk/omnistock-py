...
from app.auth.page_department import list_departments_for_admin, page_resolve_department
...
def index():
    ...
    departments = list_departments_for_admin(conn) if is_admin else []
    entries = workstation.get_for_department(conn, department["departmentId"])

    return render_template(
        "workstation/index.html", department=department, is_admin=is_admin,
